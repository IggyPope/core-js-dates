...
function getWeekNumberByDate(date) {
  ...
  const firstDayOfFirstWeek = new Date(
    firstDayOfYear - ((firstDayOfYear.getDay() || 7) - 1) * 24 * 60 * 60 * 1000
  );
  const daysPassed = (date - firstDayOfFirstWeek) / (24 * 60 * 60 * 1000);
...
function getNextFridayThe13th(date) {
  ...
  if (nextFriday13th.getDay() < 13) {
    nextFriday13th.setMonth(nextFriday13th.getMonth(), 13);
  } else {
    nextFriday13th.setMonth(nextFriday13th.getMonth() + 1, 13);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startDateTuple = period.start.split('-').map(Number);
  const endDateTuple = period.end.split('-').map(Number);
